...
module Types
  class QueryType < Types::BaseObject
    ...
    def zones_list(search_query:)
      ...
      ::V2::Zone.joins(:cache).includes(:cache).order(cumulative_infections: :desc)
                .where("search_key LIKE ? OR search_key LIKE ? OR search_key LIKE ?", "#{search_query.parameterize}%", "%-#{search_query.parameterize}%", "%;#{search_query.parameterize}%")
                .where.not(published_at: nil).limit(20)
    end
