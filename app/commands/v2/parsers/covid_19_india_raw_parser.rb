...
module V2
  module Parsers
    class Covid19IndiaRawParser < ::BaseCommand
      ...
      def parse_streams
        rows = raw_data.filter { |row| row["detectedstate"].present? }

        state_time_series = announced_time_series_for(rows)
        @streams << stream_for(unit_code: country_code, category: "announce", time_series: state_time_series)

        grouped_by_state = rows.group_by { |row| row["detectedstate"] }
        grouped_by_state.map { |state_name, s_rows| process_state(state_name: state_name, rows: s_rows) }
      end

      def process_state(state_name:, rows:)
        return true unless states_map.keys.include?(state_name.downcase)

        state_time_series = announced_time_series_for(rows)
        unit_code = states_map[state_name.downcase].code
      ...
      def process_district(state_code:, district_name:, rows:)
        return true unless districts_grouped_by_parent_code[state_code]&.include?(district_name.downcase)
